...
public interface Components {
	 ...
	 * mit der Verantwortung der Verwaltung der im Lager befindlichen (bestellbaren) Artikel.
	 */
	...
	public interface InventoryManager {
		...
		/**
		 * Calculates and returns total value of inventory.
		 * 
		...
		/**
		 * Returns inventory with <= units in store
		 * 
		 * @param units limit at or below which inventory is returned
		 * @return stream of inventory at or below units in store
		 */
		Stream<Article> inventoryAtOrLess( int units );

		/**
		 * Returns inventory with highest inventory value.
		 * 
		 * @param limit number of inventory items returned
		 * @return stream of inventory with highest inventory value, up to limit items are returned
		 */
		Stream<Article> inventoryTopRanksByValue( int limit );
	 ...
	 *– eine neue Komponente, die für die Verarbeitung von Bestellungen zuständig 
     *ist und z.B. prüft, ob alle in einer Bestellung vorkommenden Bestellpositionen auch erfüllbar 
     *sind (d.h. im Lager vorhanden) und nur in diesem Fall eine Bestellung akzeptiert. 
	 */
	public interface OrderProcessor {
		 ...
		 * rates such as rate_1: 19%, rate_2: 7%.
		 * 
		 * @param grossValue value of which included VAT is calculated
	 ...
	 * datamodel Objects.
	 *– eine System‐Komponente, die für die Erzeugung der Geschäftsobjekte (Kunden, 
     * Artikel, Bestellungen, … ) zuständig ist. 
	 */
	...
	public interface OutputProcessor {
		 ...
		 * für Formatierungs‐ und Ausgabefunktionen. 
		 */
		 ...
		 * 
		 * @param sortedByInventoryValue print sorted inventory, if true
		 */
